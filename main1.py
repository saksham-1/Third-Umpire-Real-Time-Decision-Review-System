...
stream = cv2.VideoCapture("Cricket_CLIP.mp4")
flag = True
def play(speed):
    global flag
...
def pending(decision):
    ...
    frame = cv2.cvtColor(cv2.imread("Sponser-1.png"), cv2.COLOR_BGR2RGB)
    frame = imutils.resize(frame, width=SET_WIDTH, height=SET_HEIGHT)
    frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
    canvas.image = frame
    canvas.create_image(0,0, image=frame, anchor=tkinter.NW)
...
window = tkinter.Tk()
window.title(" Saksham Real Time Third Umpire Decision Review Kit")
